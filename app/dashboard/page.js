  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-5">
        ...
        <Link
          href="/students/add"
          className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-sm"
        >
          + Student
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        >
          ...
          <div className="text-xs text-green-500 mt-1">Faculty-wise →</div>
        </Link>
      </div>
      ...
      {Object.keys(facultyCount).length > 0 && (
        <details className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
          <summary className="font-semibold text-gray-900 text-sm cursor-pointer">
            ...
            <span className="text-green-600 font-normal">click to view</span>
          </summary>
          <div className="grid grid-cols-2 gap-2 mt-3">
            {Object.entries(facultyCount).map(([faculty, count]) => (
              <div
                key={faculty}
                className="bg-green-50 rounded-lg px-3 py-2 flex justify-between items-center"
              >
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <a
                key={action.href}
                href={action.href}
                className="flex items-center text-xs text-green-600 font-medium bg-green-50 rounded-lg px-3 py-2.5 hover:bg-green-100"
              >
                {action.label}
              </a>
            ))}
              ...
              {upcomingExams.map((exam) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {exam.course}{" "}
                      {exam.semester ? `· Year ${exam.semester}` : ""} ·{" "}
                      {exam.subject}
                    </p>
                  </div>
