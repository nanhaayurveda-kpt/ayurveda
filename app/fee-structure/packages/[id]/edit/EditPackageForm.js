    ...
    <form method="POST" action={`/api/fee-structure/packages/${pkg.id}/edit`}
          ...
          {FIXED_TYPES.map((ft) => (
            <div key={ft.value}
              className={`border rounded-lg px-3 py-2.5 flex items-center gap-3 ${
                checkedTypes[ft.value] ? "border-green-400 bg-green-50" : "border-gray-200 bg-white"
              }`}>
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          <button type="button" onClick={addCustomItem}
            className="text-xs text-green-600 font-medium">+ Add Item</button>
        </div>
      ...
      {itemCount > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-lg px-4 py-3 flex justify-between items-center">
          <span className="text-sm font-medium text-green-700">Total ({itemCount} items)</span>
