  ...
  return (
    <div className="pt-2">
      ...
      <div className="mb-5">
        ...
        <span className="inline-block mt-1 text-xs font-semibold uppercase tracking-wide text-white bg-green-500 px-2 py-0.5 rounded-full">
          Principal
        </span>
      </div>

      {/* Settings incomplete alert */}
      {settingsIncomplete && (
        <Link
          href="/settings"
          className="block bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-5"
        >
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        >
          ...
          <div className="text-xs text-green-500 mt-1">Student-wise →</div>
        </Link>
        <Link
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5">
        {TILES.map((tile) => (
          <Link
            key={tile.href}
            href={tile.href}
            className="bg-white border border-gray-200 rounded-2xl p-4 flex flex-col gap-2 active:scale-95 transition hover:border-green-300 hover:bg-green-50"
          >
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl bg-green-100 text-green-700">
              {tile.icon}
            </div>
            <div>
