...
export const clinical_logs = sqliteTable("clinical_logs", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  student_id: integer("student_id").references(() => students.id),
  user_id: integer("user_id").references(() => users.id),
  date: text("date").notNull(),
  department: text("department").notNull(),
  case_type: text("case_type").notNull().default("opd"),
  patient_age: text("patient_age"),
  diagnosis: text("diagnosis").notNull(),
  procedures: text("procedures"),
  remarks: text("remarks"),
  verified_by: integer("verified_by").references(() => professors.id),
  status: text("status").notNull().default("pending"),
  created_at: integer("created_at", { mode: "timestamp" }).defaultNow(),
});

export const internship_postings = sqliteTable("internship_postings", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  student_id: integer("student_id").references(() => students.id),
  user_id: integer("user_id").references(() => users.id),
  department: text("department").notNull(),
  ...
  supervisor_id: integer("supervisor_id").references(() => professors.id),
...
export const BAMS_SUBJECTS = [
  ...
  "Maulik Siddhant evam Ashtang Hridayam",
  // 2nd Professional
  "Dravyaguna Vigyan",
  ...
  "Kaumarbhritya evam Balroga",
  // 4th Professional
  "Kayachikitsa (Advanced)",
  "Rasayana evam Vajikarana",
  "Manasa Roga",
  "Panchkarma (Advanced)",
  "Samhita Adhyayan (Advanced)",
  // Common
  "Community Medicine",
