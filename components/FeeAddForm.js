...
const FIXED_SLUGS = new Set(["tuition", "admission", "practical", "hostel", "library", "misc"]);
...
export default function FeeAddForm({
  ...
  function updateSemesterAmount(sem, feeType, val) {
    setSemesterItems((prev) => ({ ...prev, [sem]: { ...prev[sem], [feeType]: val } }));
  }
      ...
      {selectedStudentId !== "" && studentPackages.length > 0 && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Select Professional Year <span className="text-red-500">*</span>
          </label>
          <div className="space-y-3">
            {studentPackages.map((pkg) => {
              ...
              const items = semesterItems[sem] || {};
              return (
                <div key={pkg.id} className={`border rounded-lg px-3 py-2.5 ${isSelected ? "border-green-400 bg-green-50" : "border-gray-200"}`}>
                  <div className="flex items-center justify-between cursor-pointer" onClick={() => toggleSemester(sem)}>
                    <div className="flex items-center gap-2">
                      <input type="checkbox" checked={isSelected} onChange={() => toggleSemester(sem)} className="w-4 h-4 accent-green-600" />
                      <span className="text-sm font-medium text-gray-800">{sem}</span>
                    </div>
                    <span className="text-xs text-green-600 font-medium">₹{pkg.total_amount}</span>
                  </div>
                  {isSelected && (
                    <div className="mt-2 space-y-1.5 pt-2 border-t border-green-200">
                      {FIXED_TYPES.map((ft) => {
                        ...
                        return (
                          <div key={ft.value} className="flex items-center gap-2">
                            <input type="checkbox" checked={isChecked} onChange={() => toggleItem(sem, ft.value)} className="w-3.5 h-3.5 accent-green-600" />
                            {isChecked && <input type="hidden" name={`sem_${sem}_fee_type_${ft.value}`} value={ft.value} />}
                            ...
                            <input
                              ...
                              placeholder="₹"
                              disabled={!isChecked}
                              className={`w-24 border rounded-lg px-2 py-1.5 text-xs text-right focus:outline-none focus:ring-2 focus:ring-green-500 ${!isChecked ? "bg-gray-100 border-gray-200 text-gray-400" : "border-gray-300"}`}
                            />
                          </div>
                        );
                      })}
      ...
      {grossTotal > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-lg px-4 py-3 flex justify-between">
          <span className="text-sm font-medium text-green-700">
            Total ({selectedSemesters.length} year{selectedSemesters.length !== 1 ? "s" : ""})
          </span>
